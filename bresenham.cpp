/* 
 * Jose Vicente D. Chan Jr.
 * jchan81@csu.fullerton.edu
 * CWID - 892984154
 * CS 484
 *
...
private:
  ...
  void RasterizeLine( ){
    ...
    else if((delta_x > 0 && delta_y > 0) && (delta_y == delta_x)){
      theBigD = 2 * delta_y - delta_x;
    }
    else if((delta_x < 0 && delta_y < 0) && (delta_y == delta_x)){
      theBigD = 2 * abs(delta_y) - abs(delta_x);
    }
    else if((delta_x < 0 && delta_y > 0) && (delta_y == abs(delta_x))){      
      theBigD = 2 * delta_y + delta_x;
    }
    else if((delta_x > 0 && delta_y < 0) && (delta_x == abs(delta_y))){
      theBigD = -2 * delta_y - delta_x;
    }
    ...
    //Case when the angle of the slope is zero or less than equal to 45 degrees
    else if((delta_x > 0 && delta_y > 0) && (delta_x > delta_y)){
      theBigD = (2 * delta_y) - delta_x;
    }
    
    //Case when the angle of the slope is more than 45 degrees
    else if((delta_x > 0 && delta_y > 0) && (delta_y > delta_x)){
      theBigD = (2 * delta_x) - delta_y;
    }
    ...
    //Case when the angle of the slope is zero or less than equal to 135 degrees
    else if((delta_x < 0  && delta_y > 0) && (abs(delta_x) > delta_y)){
      theBigD = (2 * delta_y) + delta_x;
    }
    
    //Case when the angle of the slope is more than 135 degrees
    else if((delta_x < 0  && delta_y > 0) && (delta_y > abs(delta_x))){
      theBigD = (-2 * delta_x) - delta_y;
    }
    ...
    //Case when the angle of the slope is less than or equal to 215 degrees
    else if((delta_x < 0 && delta_y < 0) && (abs(delta_x) > abs(delta_y))){
      theBigD = (2 * abs(delta_y)) - abs(delta_x);
    }
    
    //Case when the angle of the slope is greater than 215 degrees
    else if((delta_x < 0 && delta_y < 0) && (abs(delta_y) > abs(delta_x))){
      theBigD = (2 * abs(delta_x)) - abs(delta_y);
    }
    ...
    //Case when the angle of the slope is less than or equal to 315 degrees
    else if((delta_x > 0 && delta_y < 0) && (abs(delta_x) > abs(delta_y))){
      theBigD = (2 * abs(delta_y)) - delta_x;
    }

    //Case when the angle of the slope is greater than 315 degrees
    else if((delta_x > 0 && delta_y < 0) && (abs(delta_y) > delta_x)){
      theBigD = (2 * delta_x) - abs(delta_y);
    }
    ...
    while(isDone == false){
      ...
      /* START QUADRANT I CASES */
      
      //--When slope is less than or equal to 1
	...
	if(theBigD <= 0){
	  ...
	  theBigD = theBigD + (2 * delta_y);
	}
	else if(y != y1){
	  y = y + 1;
	  theBigD = theBigD + 2 * (delta_y - delta_x);
	}
      }
	...
	if(theBigD <= 0){
	  theBigD = theBigD  + 2 * delta_x;
	}
	else if(delta_x != 0){
	  x = x + 1;
	  theBigD = theBigD + 2 * (delta_x - delta_y);
	}
      }
      /* END QUADRANT I CASES */

      /* START QUADRANT  II CASES */
	...
	if(theBigD <= 0){
      ...
      /* END QUADRANT II CASES */

      /* START QUADRANT III CASES */
	...
	if(theBigD <= 0){
	  theBigD = theBigD + (2 * abs(delta_y));
	}
	else if(y != y1){
	  y = y + 1;
	  theBigD = theBigD + 2 * (abs(delta_y) - abs(delta_x));
	}
      }
	...
	if(theBigD <=  0){
	  theBigD = theBigD + (2 * abs(delta_x));
	}
	else if(abs(delta_x) != 0){
	  x = x + 1;
	  theBigD = theBigD + 2 * (abs(delta_x) - abs(delta_y));
	}
      }

      /* END QUADRANT III CASES */

      /* START QUADRANT IV CASES */
	...
	if(theBigD <= 0){
	  ...
	  y = y + 1;
	  theBigD = theBigD + 2 * (abs(delta_y) - delta_x);
	}
      }
	...
	if(theBigD <= 0){
      ...
      //Case to take care of the diagonals
      else if((delta_x > 0 && delta_y < 0) && (delta_x == abs(delta_y))){
	if(x != x1)
	  x = x - 1;
	...
	if(y != y1)
      ...
      //Case for downward vertical line
      else if(delta_x == 0 && delta_y < 0){
	if( y != y1)
	  y = y - 1;
      ...
      //Case for right side horizontal line
      else if(delta_y == 0 && delta_x > 0){
	if(x != x1)
	  x = x + 1;
      }
      
      //Case for the left side horizontal line
      else if(delta_y == 0 && delta_x < 0){
	if(x != x1)
	  x = x - 1;
      }
